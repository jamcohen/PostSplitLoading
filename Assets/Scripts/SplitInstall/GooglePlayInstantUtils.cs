...
namespace PostSplitLoading
{
    ...
    /// </summary>
    public static class PostSplitLoadingUtils
    {
        /// <summary>
        ...
        {
            Regex extractApi = new Regex("API-(?<api>\\d+)");
            var api = extractApi.Match(operatingSystem).Groups["api"].Value;
            int version = int.Parse(api);
            return version >= 26;
        }
    }
}