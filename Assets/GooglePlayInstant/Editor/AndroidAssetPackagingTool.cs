...
namespace GooglePlayInstant.Editor
{
    ...
    public static class AndroidAssetPackagingTool
    {
        ...
        /// <summary>
        /// Given a path to a resource (xml, png, etc.), compiles them into .flat files and places them in outputPath
        /// </summary>
        /// <returns></returns>
        public static string Compile(string resourcePath, string outputPath)
        {
            var arguments = string.Format(
                "compile {0} -o {1}",
                CommandLine.QuotePath(resourcePath),
                CommandLine.QuotePath(outputPath));
            var result = CommandLine.Run(GetAapt2Path(), arguments);
            return result.exitCode == 0 ? null : result.message;
        }
        
        public static string Link(string manifestPath, string androidJarPath, string assetsPath, string baseApk, string resourcePath
            , string outputPath)
        {
            var arguments = string.Format(
                "link --proto-format --auto-add-overlay --manifest {0} --min-sdk-version {1} --version-code {2} --version-name {3} -I {4} -A {5} -I {6} -o {7}",
                ...
                CommandLine.QuotePath(baseApk),
                CommandLine.QuotePath(outputPath));
            /*var arguments = string.Format(
                "link --proto-format --auto-add-overlay --manifest {0} --min-sdk-version {1} --version-code {2} --version-name {3} -I {4} -A {5} -I {6} -R {7} -o {8}",
                CommandLine.QuotePath(manifestPath),
                (int) PlayerSettings.Android.minSdkVersion,
                PlayerSettings.Android.bundleVersionCode,
                PlayerSettings.bundleVersion,
